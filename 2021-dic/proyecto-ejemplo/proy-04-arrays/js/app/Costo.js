...
class Costo {
    constructor(props) {
        ...
        this.seccionesDeDesarrollo = props.seccionesDeDesarrollo;
        ...
        this.costosPorSeccion = [];
    }

    cargarCostosPorHora() {
        if (this.seccionesDeDesarrollo.length > 0) {
    ...
    calcularCostosPorSeccion() {
        this.costosPorSeccion = this.seccionesDeDesarrollo.map((objSeccion) => {
            return {
                nombre: objSeccion.nombre,
                horas: objSeccion.horas,
                monto: convertirDecimal2(objSeccion.horas * this.precioHoraDev, true)
            }
        })
    }
    ...
    mostrarCostosPorSeccion() {
        mostrarMsg('=========================== Secciones =====================');
        for (let i = 0; i < this.costosPorSeccion.length; i++) {
            const objSeccion = this.costosPorSeccion[i];
            mostrarMsg('Sección: ' + objSeccion.nombre + ' - costo: $' + objSeccion.monto);
        }
        mostrarMsg('===========================================================');
        // Una alternativa al uso de for:     
        // this.costosPorSeccion.forEach((objSeccion) => {
        //     mostrarMsg('Sección: ' + objSeccion.nombre + ' - costo: $' + objSeccion.monto);
        // })
    }
