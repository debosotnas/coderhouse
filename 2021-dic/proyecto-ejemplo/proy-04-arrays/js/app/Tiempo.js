...
import { obtenerEntero, mostrarMsg, obtenerArray } from "./utils.js";
...
class Tiempo {
    ...
    cargarSeccionesyHorasDeDesarrollo() {
        let horasTotalesDev = 0;
        let arrSeccion = [];
        do {
            arrSeccion = obtenerArray('Ingrese los siguientes valores separados por coma (o ESC para terminar): Nombre, Cantidad de Horas');
            if (arrSeccion.length > 1) {
                const objSeccion = {
                    nombre: arrSeccion[0],
                    horas: parseInt(arrSeccion[1])
                }
                this.seccionesDeDesarrollo.push(objSeccion);
                horasTotalesDev += objSeccion.horas;
            }
        } while (arrSeccion[0] !== 'ESC' && arrSeccion[0] !== 'esc');
        this.cantidadEstimadaDeHorasDev = horasTotalesDev;
    }
